...
def can_push(
    box: tuple[int, int], dy: int, grid: dict[tuple[int, int], GridObj]
) -> bool:
    x, y = box
    y = y + dy
    if (x, y) not in grid:
...
def push(
    box: tuple[int, int], obj: GridObj, dy: int, grid: dict[tuple[int, int], GridObj]
):
    x, y = box
    y = y + dy
    if not (x, y) in grid:
...
def part_2(data: Data):
    ...
    for move in moves:
        ...
        if dy == 0:
            next_crate = new_robot
            while next_crate in grid and grid[next_crate] in (
                GridObj.CRATE_L,
                GridObj.CRATE_R,
            ):
