...
def get_new_range(part, rule):
    if rule.op_sym == '<':
        if part[rule.label][0] >= rule.value: # Part outside of rule
            ...
            return part | {
                rule.label: [part[rule.label][0], min(rule.value - 1, part[rule.label][1])]
            }       
    else: # rule.op_sum == '>'
        if part[rule.label][1] <= rule.value: # Part outside of rule
            ...
            return part | {
                rule.label: [max(rule.value + 1, part[rule.label][0]), part[rule.label][1]]
            }
...
def part_2_recurse(state: State, parts: list[dict[str, list[int]]], states: dict[str, State]):
    if state.label == 'in':
        return parts
    parent_states = filter(lambda t_state: any(map(lambda rule: rule.target == state.label, t_state.rules)), states.values())
...
def part_2(data: Data):
    states = data['states']
    accepted_states = filter(lambda state: any(map(lambda rule: rule.target == 'A', state.rules)), states.values())
