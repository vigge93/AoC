...
def get_next_obstacle(
    beam: tuple[int, int, str], obstacles: dict[str, dict[int, list[Obstacle]]]
) -> Obstacle:
    x, y, direction = beam
    if direction == "s":
...
def energize_counter(
    beam: tuple[int, int, str],
    energized: set[tuple[int, int, str]],
    obstacles: dict[str, dict[int, list[Obstacle]]],
):
    ...
    if not next_obstacle:
        if direction == "s":
            energized.update(
                [(x, new_y, direction) for new_y in range(y, max(obstacles["s"]) + 1)]
            )
        elif direction == "n":
            energized.update([(x, new_y, direction) for new_y in range(y, -1, -1)])
        elif direction == "e":
            energized.update(
                [(new_x, y, direction) for new_x in range(x, max(obstacles["e"]) + 1)]
            )
        elif direction == "w":
    ...
    if direction == "s":
        ...
        energized.update(
            [(x, new_y, direction) for new_y in range(y, next_obstacle.y, -1)]
        )
        if next_obstacle.type in ("/", "-"):
            energize_counter((next_x, next_y, "e"), energized, obstacles)
        if next_obstacle.type in ("\\", "-"):
        ...
        energized.update(
            [(new_x, y, direction) for new_x in range(x, next_obstacle.x, -1)]
        )
        if next_obstacle.type in ("/", "|"):
            energize_counter((next_x, next_y, "s"), energized, obstacles)
        if next_obstacle.type in ("\\", "|"):
